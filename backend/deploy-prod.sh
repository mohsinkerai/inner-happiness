...
java -jar qa-build.jar --server.port=8090 --spring.datasource.password=${DB_PROD_PW} --spring.datasource.host=${DB_PROD_HOST} --spring.datasource.username=${DB_PROD_USER} --spring.datasource.db=inner_satisfaction -Xmx600m > startup_prod_log.out 2>&1 &
EOF
