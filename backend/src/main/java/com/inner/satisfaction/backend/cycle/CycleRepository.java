...
public interface CycleRepository extends BaseRepository<Cycle> {
  ...
  String findPositionsWithoutRecommendation =
    ...
    + " FROM "
    + "  appointment_position ap "
    + "    LEFT OUTER JOIN person_appointment pa ON ap.id = pa.appointment_position_id "
    + "    INNER JOIN institution i ON i.id = ap.institution_id "
    + "    INNER JOIN position p ON p.id = ap.position_id "
    + " WHERE "
    + "  ap.cycle_id = ?1 AND (ap.`state` = null or ap.`state` = 'CREATED') AND ap.is_active=true"
    + " GROUP BY "
    + "  ap.id, p.name, i.name "
    + " HAVING "
