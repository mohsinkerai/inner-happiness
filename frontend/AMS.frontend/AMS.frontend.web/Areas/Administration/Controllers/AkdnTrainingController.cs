...
namespace AMS.frontend.web.Areas.Administration.Controllers
{
    ...
    public class AkdnTrainingController : BaseController
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Index(AkdnTraineeModel model)
        {
            if (ModelState.IsValid)
            {
                var success = await new RestfulClient(HttpContext.Session.Get<AuthenticationResponse>("AuthenticationResponse")?.Token).AddNewAkdnTraining(model);
                if (success)
                {
