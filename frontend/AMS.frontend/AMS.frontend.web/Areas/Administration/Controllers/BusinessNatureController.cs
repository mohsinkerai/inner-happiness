...
namespace AMS.frontend.web.Areas.Administration.Controllers
{
    ...
    public class BusinessNatureController : BaseController
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Index(BusinessNatureModel model)
        {
            if (ModelState.IsValid)
            {
                var success = await new RestfulClient(HttpContext.Session.Get<AuthenticationResponse>("AuthenticationResponse")?.Token).AddNewBusinessNature(model);
                if (success)
                {
