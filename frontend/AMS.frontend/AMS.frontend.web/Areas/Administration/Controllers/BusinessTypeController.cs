...
namespace AMS.frontend.web.Areas.Administration.Controllers
{
    ...
    public class BusinessTypeController : BaseController
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Index(BusinessTypeModel model)
        {
            if (ModelState.IsValid)
            {
                var success = await new RestfulClient(HttpContext.Session.Get<AuthenticationResponse>("AuthenticationResponse")?.Token).AddNewBusinessType(model);
                if (success)
                {
