...
namespace AMS.frontend.web.Areas.Administration.Controllers
{
    ...
    public class FieldOfExpertiseController : BaseController
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Index(FieldOfExpertiseModel model)
        {
            if (ModelState.IsValid)
            {
                var success = await new RestfulClient(HttpContext.Session.Get<AuthenticationResponse>("AuthenticationResponse")?.Token).AddNewFieldOfExpertise(model);
                if (success)
                {
