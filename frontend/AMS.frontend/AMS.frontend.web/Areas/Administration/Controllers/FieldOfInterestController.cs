...
namespace AMS.frontend.web.Areas.Administration.Controllers
{
    ...
    public class FieldOfInterestController : BaseController
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Index(FieldOfInterestModel model)
        {
            if (ModelState.IsValid)
            {
                var success = await new RestfulClient(HttpContext.Session.Get<AuthenticationResponse>("AuthenticationResponse")?.Token).AddNewFieldOfInterest(model);
                if (success)
                {
